...
class GoogleAdsConnector extends GoogleConnector
{
    /**
     * @var GoogleAdsClientBuilder $googleAds
     */
    private $googleAds;
    ...
    public function __construct($params)
    {
        ...
        $oAuth2Credential = (new OAuth2TokenBuilder())
            ...
            ->build();
        $this->googleAds = (new GoogleAdsClientBuilder())
            ->withOAuth2Credential($oAuth2Credential)
            ->withDeveloperToken($this->developerKey)
            ->build();
    }

    /**
     * Get ListAccessible Accounts Ads
     * @return array
     */
    public function getAdsAccounts() {
        ...
        $result = [];
        try {
            $customerServiceClient = $this->googleAds->getCustomerServiceClient();
            ...
            foreach ($accessibleCustomers->getResourceNames() as $resourceName) {
                $resource = $this->getUserExtraData($resourceName);
                if(!empty($resource)){
            ...
            foreach ($googleAdsException->getGoogleAdsFailure()->getErrors() as $error) {
                ...
                $errore .= $error->getErrorCode()->getErrorCode() . $error->getMessage() . PHP_EOL;
            }
            echo $this->setError($errore);
        } catch (ApiException $apiException) {
            echo $this->setError($apiException->getMessage());
        }
    ...
    /**
     * Get Accont Information
     * @param $resourceName
     * @return array|null
     */
    private function getUserExtraData($resourceName) {
        $result = null;
        try{
            $customerServiceClient = $this->googleAds->getCustomerServiceClient();
            ...
            $result['name'] = (isset($customerName)) ? $customerName->getValue() : $customerId;
        } catch (GoogleAdsException $googleAdsException) {
            $errore = null;
            foreach ($googleAdsException->getGoogleAdsFailure()->getErrors() as $error) {
                /** @var GoogleAdsError $error */
            ...
            $this->setError($apiException->getMessage());
        }
